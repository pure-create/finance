...
function incomeAt(cfg, startAge) {
  ...
  const pensionFrom = isFinite(cfg.pensionFrom)
    ? cfg.pensionFrom
    : cfg.ageRetire;
  let v = 0;
...
function simulate(cfg) {
  ...
  // 各資産の対数正規パラメータ
  const lp = [0, 1, 2].map(function (i) {
    return logParams(cfg.ret[i] / 100, cfg.risk[i] / 100);
  });
  ...
  for (let t = 0; t < T; t++) {
    ...
    for (let y = 1; y <= N; y++) {
      ...
      if (startAge < cfg.ageRetire) {
        ...
        if (y === N)
          finalWithdrawReal[t] = netTaken / Math.pow(1 + infl, y - 1);
      }
  ...
  const depSorted = depletionAge.slice().sort(function (a, b) {
    return a - b;
  });
  ...
  return {
    ...
    realFactor: function (y) {
      return Math.pow(1 + infl, y);
    },
    weights: w,
  };
}
...
function portfolioStats(alloc, ret, risk, corr) {
  ...
  let v = 0;
  for (let i = 0; i < 3; i++)
    for (let j = 0; j < 3; j++) v += w[i] * w[j] * risk[i] * risk[j] * c[i][j];
