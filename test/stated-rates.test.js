...
/* 表のセルなど、HTML構造そのものを見るテスト用。整形ツールがタグ間へ
   改行を入れても、構造が同じなら同じマークアップとして扱う。 */
const markup = (rel) => read(rel).replace(/>\s+</g, "><");
...
function ratioEquals(percentText, rate, msg) {
  assert.ok(
    Math.abs(Number(percentText) / 100 - rate) < 1e-12,
...
test("資産運用：NISAの投資枠の説明が定数と一致する", () => {
  ...
  // 入力欄の上限も生涯投資枠に合わせてある
  const m3 = must(
    read("assetSimulator/index.html"),
    /id="nisaUsed"[^>]*max="(\d+)"/,
    "assetSimulator/index.html",
  );
  assert.strictEqual(
...
test("相続：基礎控除の式が計算と一致する", () => {
  const m = must(
    prose("inheritance/index.html"),
    /基礎控除は「([\d,]+)万円＋([\d,]+)万円×法定相続人の数」/,
  ...
  for (const heirs of [1, 2, 3, 5]) {
    ...
    assert.strictEqual(
      inherit.totalTax(ded, hasSpouse, nChildren),
      0,
      "法定相続人" + heirs + "人：基礎控除ちょうどで課税されている",
    );
    assert.ok(
      inherit.totalTax(ded + 1, hasSpouse, nChildren) > 0,
...
test("相続：小規模宅地等の特例の説明が定数と一致する", () => {
  ...
  const note = must(
    text,
    /特定居住用宅地等（(\d+)㎡までの部分を(\d+)%減額）/,
    "inheritance/index.html（注記）",
  );
  assert.strictEqual(
    Number(note[1]),
    inherit.SMALL_LOT_LIMIT,
    "注記の限度面積",
  );
...
test("相続：生命保険金の非課税枠の説明が定数と一致する", () => {
  ...
  assert.strictEqual(
    yen(tip[1]),
    inherit.INSURANCE_PER_HEIR,
    "1人あたりの非課税枠",
  );

  const note = must(
    text,
    /生命保険金の非課税枠（([\d,]+)万円×法定相続人の数）/,
    "inheritance/index.html（注記）",
  );
  assert.strictEqual(
    yen(note[1]),
  ...
  for (const heirs of [1, 3, 5]) {
    assert.strictEqual(
      inherit.insuranceExemption(1e9, heirs),
      inherit.INSURANCE_PER_HEIR * heirs,
      "法定相続人" + heirs + "人の枠",
    );
  }
});
...
const RETIRE_PAGES = [
  "retirement/local/index.html",
  "retirement/national/index.html",
];
...
test("退職手当：所得税の速算表がcalcTaxと一致する", () => {
  for (const page of RETIRE_PAGES) {
    ...
    assert.ok(
      rows.length >= 7,
      page + " の速算表を読み取れていない（" + rows.length + "行）",
    );
    ...
    for (const r of rows) {
      ...
      const koujo = 800000;
      const kazei =
        hi === null ? lo + 1000000 : Math.floor((lo + hi) / 2 / 1000) * 1000;
...
test("退職手当：速算表の区分の切れ目がcalcTaxの切り替わりと一致する", () => {
  for (const page of RETIRE_PAGES) {
    ...
    for (const r of rows) {
      ...
      assert.ok(
        Math.abs(atLo - Math.floor((lo * rate - ded) * 1.021)) <= 1,
      ...
      assert.ok(
        Math.abs(belowActual - belowByThisRow) > 1,
        page +
          "：" +
          r[1] +
          "円の1つ下も同じ税率で計算されている（区分の切れ目がずれている）",
      );
    }
  }
});
...
test("退職手当：退職所得控除の説明がretireDeductionと一致する", () => {
  for (const page of RETIRE_PAGES) {
    ...
    const short = must(
      html,
      /<th>(\d+)年以下<\/th><td>(\d+)万円×勤続年数（最低(\d+)万円）<\/td>/,
      page,
    );
    const border = Number(short[1]),
      perYear = Number(short[2]) * 10000,
      floor = Number(short[3]) * 10000;

    const long = must(
      html,
      /<th>(\d+)年超<\/th><td>(\d+)万円×\(勤続年数 - (\d+)年\) \+ (\d+)万円<\/td>/,
      page,
    );
    assert.strictEqual(
      Number(long[1]),
      border,
      page + "：短期と長期の境目が食い違っている",
    );
    assert.strictEqual(
      Number(long[3]),
      border,
    ...
    const perYearLong = Number(long[2]) * 10000,
      baseLong = Number(long[4]) * 10000;
...
test("退職手当：定年の引き上げの説明がteinenAgeと一致する", () => {
  for (const page of RETIRE_PAGES) {
    ...
    const m = must(
      html,
      /(\d{4})年度から(\d{4})年度まで<br \/>段階的に定年が(\d+)歳まで<br \/>引き上げられます/,
      page,
    );
    ...
    assert.strictEqual(
      retire.teinenAge(from - 1),
      60,
      page + "：引き上げ開始の前年が60歳でない",
    );
    assert.ok(
      retire.teinenAge(from) > 60,
      page + "：引き上げ開始年に上がっていない",
    );
    assert.strictEqual(
      retire.teinenAge(until),
      goal,
      page + "：終了年度に" + goal + "歳へ届いていない",
    );
    assert.ok(
      retire.teinenAge(until - 1) < goal,
      page + "：終了年度より前に到達している",
    );
    assert.strictEqual(
      retire.teinenAge(until + 20),
      goal,
      page + "：" + goal + "歳で頭打ちになっていない",
    );
  }
});
...
test("年金：繰上げ・繰下げの増減率の説明がpRateと一致する", () => {
  ...
  const downPerMonth = Number(down[1]) / 100,
    downMax = Number(down[2]) / 100;
  const upPerMonth = Number(up[1]) / 100,
    upMax = Number(up[2]) / 100;

  // 1か月ぶんの増減が書かれている率どおりか
  assert.ok(
    Math.abs(pension.pRate(65) - pension.pRate(65 - 1 / 12) - downPerMonth) <
      1e-12,
    "繰上げの1か月あたりの減額率",
  );
  assert.ok(
    Math.abs(pension.pRate(65 + 1 / 12) - pension.pRate(65) - upPerMonth) <
      1e-12,
    "繰下げの1か月あたりの増額率",
  );
  ...
  assert.ok(
    Math.abs(pension.pRate(60) - (1 - downMax)) < 1e-12,
    "60歳開始の減額（最大" + down[2] + "%減）",
  );
  assert.ok(
    Math.abs(pension.pRate(75) - (1 + upMax)) < 1e-12,
    "75歳開始の増額（最大" + up[2] + "%増）",
  );
  // 75歳より後ろは増えない
...
test("年金：受給開始年齢ごとの年金額の割合が表示と一致する", () => {
  ...
  for (const age of [60, 70, 75]) {
    // 表示は整数％に丸めた値
    assert.strictEqual(
      Math.round(pension.pRate(age) * 100),
      stated[age],
      age + "歳開始の年金額の割合",
    );
  }
  assert.strictEqual(
    Math.round(pension.pRate(65) * 100),
    100,
    "65歳開始が基準（100%）",
  );
});
...
test("年金：想定利回りのスライダーの範囲が説明と一致する", () => {
  ...
  const slider = must(
    read("pension/index.html"),
    /id="rSlider"[^>]*min="([\d.]+)"[^>]*max="([\d.]+)"/,
    "pension/index.html",
  );
...
test("iDeCo：掛金相当額の説明にある第2号の枠が定数と一致する", () => {
  const m = must(
    prose("ideco/index.html"),
    /第2号被保険者の枠は「月([\d.]+)万円 −/,
    "ideco/index.html",
  );
  assert.strictEqual(
    Number(m[1]) * 10000,
    ideco.CONTRIBUTION_LIMITS.employee.reformed,
    "第2号の改正後の枠",
  );
});
...
test("iDeCo：注記の重複期間のルールが定数と一致する", () => {
  ...
  const first = must(
    text,
    /iDeCoを先に受け取った場合は前年以前(\d+)年内/,
    "ideco/index.html",
  );
  assert.strictEqual(
    Number(first[1]),
    ideco.OVERLAP_YEARS_IDECO_FIRST,
    "iDeCoが先の場合",
  );
  const later = must(
    text,
    /退職金を先に受け取った場合は前年以前(\d+)年内/,
    "ideco/index.html",
  );
  assert.strictEqual(
    Number(later[1]),
    ideco.OVERLAP_YEARS_RETIRE_FIRST,
    "退職金が先の場合",
  );
});
...
test("iDeCo：注記の第5号加入者の年齢が定数と一致する", () => {
  ...
  const m = must(
    text,
    /改正後は(\d+)歳以上(\d+)歳未満で国民年金の被保険者でない人も「第5号加入者」/,
    "ideco/index.html",
  );
  assert.strictEqual(
    Number(m[1]),
    ideco.NATIONAL_PENSION_END_AGE,
    "国民年金の被保険者でなくなる年齢",
  );
  assert.strictEqual(
    Number(m[2]),
  ...
  assert.strictEqual(
    ideco.LATE_JOIN_CATEGORY_LIMIT,
    ideco.CONTRIBUTION_LIMITS.employee.reformed,
  );
});
...
test("iDeCo：受給開始年齢の説明の年齢と増減率が定数と一致する", () => {
  ...
  const m = must(
    text,
    /原則は(\d+)歳ですが、(\d+)歳から(\d+)歳の間で選べます/,
    "ideco/index.html",
  );
  assert.strictEqual(
    Number(m[1]),
    ideco.PUBLIC_PENSION_START_AGE,
    "原則の受給開始年齢",
  );
  assert.strictEqual(
    Number(m[2]),
    ideco.PUBLIC_PENSION_MIN_AGE,
    "繰上げの下限",
  );
  assert.strictEqual(
    Number(m[3]),
    ideco.PUBLIC_PENSION_MAX_AGE,
    "繰下げの上限",
  );
  ...
  const r = must(
    text,
    /早めると1か月あたり([\d.]+)%減り（(\d+)歳で(\d+)%減）、遅らせると1か月あたり([\d.]+)%増えます（(\d+)歳で(\d+)%増）/,
    "ideco/index.html",
  );
  ratioEquals(r[1], ideco.PENSION_EARLY_RATE, "繰上げの1か月あたりの減額率");
  ratioEquals(r[4], ideco.PENSION_LATE_RATE, "繰下げの1か月あたりの増額率");
  assert.ok(
    Math.abs(ideco.publicPensionRate(Number(r[2])) - (1 - Number(r[3]) / 100)) <
      1e-12,
    r[2] + "歳まで早めたときの減額",
  );
  assert.ok(
    Math.abs(ideco.publicPensionRate(Number(r[5])) - (1 + Number(r[6]) / 100)) <
      1e-12,
    r[5] + "歳まで遅らせたときの増額",
  );
});
...
test("iDeCo：受給開始年齢の入力欄の範囲が定数と一致する", () => {
  const m = must(
    ...
    /id="publicPensionAge"[^>]*min="(\d+)"[^>]*max="(\d+)"/,
    "ideco/index.html",
  );
  assert.strictEqual(
    Number(m[1]),
...
test("iDeCo：繰上げ・繰下げの率が年金シミュレーターと一致する", () => {
  ...
  for (
    let age = ideco.PUBLIC_PENSION_MIN_AGE;
    age <= ideco.PUBLIC_PENSION_MAX_AGE;
    age++
  ) {
    assert.ok(
      Math.abs(ideco.publicPensionRate(age) - pension.pRate(age)) < 1e-12,
      age + "歳受給開始の増減率が2つのツールで違う",
    );
  }
  // 上限を超えたときの頭打ちも同じ
  assert.ok(
    Math.abs(ideco.publicPensionRate(80) - pension.pRate(80)) < 1e-12,
    "繰下げの頭打ち",
  );
});
...
test("iDeCo：注記の短期退職手当等の条件が定数と一致する", () => {
  ...
  const m = must(
    text,
    /加入期間が(\d+)年以下の場合は短期退職手当等にあたり、残りのうち([\d,]+)万円を超える部分/,
    "ideco/index.html",
  );
  assert.strictEqual(
    Number(m[1]),
    tax.SHORT_TENURE_YEARS,
    "短期の判定になる勤続年数",
  );
  assert.strictEqual(
    yen(m[2]) * 10000,
    tax.SHORT_TENURE_HALF_LIMIT,
    "2分の1にできる上限",
  );
});
...
test("iDeCo：区分の選択肢が定数の一覧とそろっている", () => {
  ...
  const options = [
    ...html.matchAll(
      /<option value="(self|employee|corporate|publicSv|spouse)"[^>]*>/g,
    ),
  ].map((m) => m[1]);
...
test("iDeCo：選択肢の初期値がJS側の初期値と一致する", () => {
  ...
  for (const id of ["category"]) {
    ...
    const field = js.match(
      new RegExp("\\[[\"\\']" + id + "[\"\\'],\\s*[\"\\']([^\"\\']+)"),
    );
    assert.ok(field, "ideco/js/ideco.js の FIELDS に " + id + " が無い");
    const def = field[1];
...
test("iDeCo：注記の受給開始年齢の要約が定数と一致する", () => {
  ...
  const base = must(
    text,
    /(\d+)歳から受け取るには、(\d+)歳になるまでの通算加入者等期間が(\d+)年以上必要/,
    "ideco/index.html",
  );
  assert.strictEqual(
    Number(base[1]),
  ...
  const step = must(
    text,
    /受け取れる年齢が(\d+)年ごとに繰り下がり、最も遅くて(\d+)歳から/,
    "ideco/index.html",
  );
  assert.strictEqual(
    Number(step[2]),
  ...
  for (let i = 1; i < table.length - 1; i++) {
    assert.strictEqual(
      table[i - 1].minYears - table[i].minYears,
      Number(step[1]),
...
test("iDeCo：注記の「110万円まで非課税」が公的年金等控除の表と一致する", () => {
  ...
  const m = must(
    prose("ideco/index.html"),
    /65歳以上なら合わせて([\d,]+)万円まで（65歳未満は([\d,]+)万円まで）は課税されない/,
    "ideco/index.html",
  );
  /* 60歳から年金で受け取ることもできるので、65歳未満の枠も書いてある。
	   どちらも速算表の下端（min:0 の受け皿の1つ上）と同じでなければならない */
  for (const [stated, key, age] of [
    [yen(m[1]) * 10000, "from65", 65],
    [yen(m[2]) * 10000, "under65", 64],
  ]) {
