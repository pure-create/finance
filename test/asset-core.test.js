...
function balanceAt(sim, year) {
  return sim.stats[year].p50;
}
...
test("信託報酬はリターンから差し引かれる", () => {
  const sim = simulate(
    cfg({ ageRetire: 50, ageEnd: 50, ret: [10, 0, 0], fee: 0.15 }),
  );
  // 10% − 0.15% ＝ 9.85%
  near(
    balanceAt(sim, 10),
    1000 * Math.pow(1.0985, 10),
    1e-6,
    "手数料控除後の残高",
  );
});
...
test("配分の合計が100%でなくても割合として正規化される", () => {
  const a = simulate(
    cfg({ ageRetire: 45, ageEnd: 45, alloc: [50, 50, 0], ret: [10, 2, 0] }),
  );
  const b = simulate(
    cfg({ ageRetire: 45, ageEnd: 45, alloc: [10, 10, 0], ret: [10, 2, 0] }),
  );
...
test("積立額はインフレ率で増額される", () => {
  ...
  const sim = simulate(
    cfg({ ageRetire: 43, ageEnd: 43, contribution: 100, inflation: 2 }),
  );
...
test("退職金は名目のままなら物価で増えない", () => {
  ...
  const nominal = simulate(
    cfg(Object.assign({}, base, { lumpBase: "nominal" })),
  );
...
test("売却益への課税は含み益の割合ぶんだけかかる", () => {
  ...
  assert.ok(
    balanceAt(taxed, 2) < balanceAt(free, 2),
    "課税ありのほうが残高が少ない",
  );
...
test("NISAの生涯枠を使い切っていれば、NISAを使わない場合と同じ結果になる", () => {
  ...
  const noRoom = simulate(
    cfg(Object.assign({}, common, { nisaOn: true, nisaUsed: 1800 })),
  );
...
test("NISA枠が残っていれば課税口座だけより有利になる", () => {
  ...
  const withNisa = simulate(
    cfg(Object.assign({}, common, { nisaOn: true, nisaUsed: 0 })),
  );
  const noNisa = simulate(cfg(Object.assign({}, common, { nisaOn: false })));
  assert.ok(
...
test("分位点は下位から上位へ順に並ぶ", () => {
  ...
  for (const s of sim.stats) {
    assert.ok(
      s.p05 <= s.p25 && s.p25 <= s.p50 && s.p50 <= s.p75 && s.p75 <= s.p95,
      s.age + "歳で分位点の並びが逆転している",
    );
    assert.ok(
      s.min <= s.p05 && s.p95 <= s.max,
      s.age + "歳で最小・最大が範囲外",
    );
...
test("残高がマイナスになることはない", () => {
  ...
  for (const s of sim.stats)
    assert.ok(s.min >= 0, s.age + "歳で残高がマイナス");
});
...
test("logParams：算術平均リターンとして扱われる", () => {
  ...
  for (const [r, s] of [
    ...
    near(
      Math.exp(p.mu + (p.sigma * p.sigma) / 2),
      1 + r,
      1e-12,
      "r=" + r + " s=" + s,
    );
...
test("lumpAt：受け取る年齢の年だけ、名目か実質かで額が変わる", () => {
  ...
  near(
    lumpAt({ lumpSum: 2000, lumpAge: 65, lumpBase: "real" }, 65, 1.5),
    3000,
    1e-12,
    "実質",
  );
  near(
    lumpAt({ lumpSum: 0, lumpAge: 65, lumpBase: "real" }, 65, 1.5),
    0,
    1e-12,
    "退職金なし",
  );
