...
function allHtml(dir, out) {
  ...
  for (const e of fs.readdirSync(dir, { withFileTypes: true })) {
    if (e.name === ".git" || e.name === "node_modules" || e.name === ".claude")
      continue;
...
function fileToUrl(file) {
  return (
    SITE + file.replace(/(^|\/)index\.html$/, "$1").replace(/^index\.html$/, "")
  );
}
...
test("HTMLを取りこぼさずに見ている", () => {
  assert.ok(
    HTML_FILES.length >= 10,
    "見つかったHTMLが少なすぎる: " + HTML_FILES.length,
  );
...
test("canonical を持つページは必ず sitemap.xml に載っている", () => {
  ...
  for (const file of HTML_FILES) {
    ...
    assert.ok(
      canonical,
      file +
        " に canonical が無い（意図的なら test/site-links.test.js の NO_CANONICAL へ）",
    );
...
test("canonical を持たないのは404とOGPの版下だけ", () => {
  for (const file of NO_CANONICAL) {
    ...
    assert.ok(
      !/<link\s+rel="canonical"/.test(read(file)),
      file + " に canonical が付いている",
    );
  }
});
...
test("パンくずの最後は自分自身で、名前は title と一致する", () => {
  ...
  for (const file of HTML_FILES) {
    ...
    assert.strictEqual(
      items[0].item,
      SITE,
      file + " のパンくずがトップページから始まっていない",
    );
    items.forEach((it, i) => {
      assert.strictEqual(
        it.position,
        i + 1,
        file + " のパンくずの position が飛んでいる",
      );
    });
  }
});
...
test("tool-nav.js の行き先はすべて実在し、sitemap にも載っている", () => {
  ...
  const tools = Array.from(
    nav.matchAll(
      /\{\s*label:\s*["']([^"']+)["'],\s*dir:\s*["']([^"']+)["'],\s*href:\s*["']([^"']+)["']\s*\}/g,
    ),
  ...
  for (const t of tools) {
    ...
    assert.ok(
      fs.existsSync(path.join(ROOT, file)),
      t.label + " の行き先が無い: " + t.href,
    );
    assert.ok(
      listed.has(SITE + t.href.replace(/index\.html$/, "")),
    ...
    assert.ok(
      fs.existsSync(path.join(ROOT, t.dir)),
      t.label + " の区画が無い: " + t.dir,
    );
  }
});

test("トップページと404ページと tool-nav.js が同じツールを指している", () => {
  // トップページの各カード内のリンク先（見出しリンクと補助リンクを含む）
  const top = read("index.html");
  const topLinks = new Set(
    Array.from(
      top.matchAll(/<section class="item[\s\S]*?<\/section>/g),
    ).flatMap((section) =>
      Array.from(section[0].matchAll(/<a href="([^"]+)">/g)).map((m) =>
        resolveLink("index.html", m[1]),
      ),
    ),
  );
  // 404ページのツリーの行き先
  ...
  const treeLinks = new Set(
    Array.from(notFound.matchAll(/<li>\s*<a href="([^"]+)">/g)).map((m) =>
      resolveLink("404.html", m[1]),
    ),
  );
  ...
  for (const l of topLinks) {
    assert.ok(
      treeLinks.has(l),
      l + " がトップページにあるのに404ページのツリーに無い",
    );
  }
  // 404のツリーにはトップとプライバシーポリシーも並ぶので、逆向きは差を許す
  const extra = Array.from(treeLinks).filter(
    (l) => !topLinks.has(l) && l !== "index.html" && l !== "privacy/index.html",
  );
  assert.deepStrictEqual(
    extra,
    [],
    "404ページにあってトップページに無い行き先",
  );
});

test("ツール一覧は資産形成から贈与・相続まで同じ順で並ぶ", () => {
  const order = [
    "assetSimulator/",
    "nisa/nisa2025.html",
    "ideco/",
  ...
  const sources = [
    { name: "トップページ", text: read("index.html"), prefix: 'href="' },
    { name: "404ページ", text: read("404.html"), prefix: 'href="/finance/' },
    {
      name: "共通フッター",
      text: read("common/tool-nav.js"),
      prefix: 'href: "',
    },
  ];
  ...
  for (const source of sources) {
    ...
    for (const href of order) {
      const at = source.text.indexOf(source.prefix + href, previous + 1);
      assert.ok(at > previous, source.name + "の並び順が違う: " + href);
      previous = at;
    }
  }
});
...
test("sitemap.xml のURLは canonical と同じ形（末尾のスラッシュまで）", () => {
  for (const url of SITEMAP_URLS) {
    ...
    assert.strictEqual(
      url,
      canonical,
      file + " の sitemap のURLと canonical の形が違う",
    );
  }
});
...
test("どのページにも本文へのスキップリンクと、その行き先がある", () => {
  for (const file of PAGES) {
    ...
    assert.match(
      html,
      /id="main"/,
      file + ' に id="main" が無い（スキップリンクの行き先）',
    );
  }
});
...
test("トップページ以外にはヘッダーの戻る導線があり、行き先はトップページ", () => {
  for (const file of PAGES) {
    ...
    assert.strictEqual(
      resolveLink(file, link),
      TOP,
      file + " の戻る導線がトップページを指していない",
    );
  }
});
...
test("ヘッダー行は <main> の外にあり、戻る導線はその中にある", () => {
  ...
  for (const file of PAGES) {
    ...
    assert.ok(
      main !== -1 && head < main,
      file + " のヘッダー行が <main> の中にある",
    );
    assert.ok(
      link > head && link < main,
      file + " の戻る導線がヘッダー行の中に無い",
    );
  }
});
...
test("ページ末尾の導線があり、深さの指定がトップページを指している", () => {
  ...
  for (const file of PAGES) {
    ...
    if (file === TOP || file === "404.html") {
      assert.ok(
        !base,
        file + " は行き先の一覧を自前で持つので、末尾の導線は要らない",
      );
      continue;
    }
    ...
    assert.strictEqual(
      resolveLink(file, base),
      TOP,
      file + " の data-tool-nav がトップページを指していない",
    );
  }
});
...
test("NISAの各ページの年が、データの側にもある", () => {
  ...
  const years = new Set(
    [...read("nisa/nisa-data.js").matchAll(/^\s+(\d{4}):\s*\{/gm)].map(
      (m) => m[1],
    ),
  );
  ...
  for (const file of pages) {
    ...
    assert.ok(
      years.has(year),
      file + " の年（" + year + "）が nisa-data.js に無い",
    );
...
test("横に長い表は、キーボードでも動かせる", () => {
  ...
  for (const file of PAGES) {
    const html = read(file);
    for (const m of html.matchAll(
      /<div class=['"][^'"]*table-scroll[^'"]*['"][^>]*>/g,
    )) {
      const tag = m[0];
      assert.match(
        tag,
        /tabindex=['"]0['"]/,
        file + ' の横スクロールする表に tabindex="0" が無い: ' + tag,
      );
      assert.match(
        tag,
        /role=['"]region['"]/,
        file + ' の横スクロールする表に role="region" が無い: ' + tag,
      );
      assert.match(
        tag,
        /aria-label=['"][^'"]+['"]/,
        file + " の横スクロールする表に aria-label が無い: " + tag,
      );
    }
  }
});
...
function excludeList() {
  return [...read("_config.yml").matchAll(/^\s*-\s*([^\s#]+)/gm)].map((m) =>
    m[1].replace(/\/$/, ""),
  );
}
...
test("_config.yml がテストと npm の設定を公開対象から外している", () => {
  ...
  for (const name of ["test", "package.json"]) {
    assert.ok(
      ...
      "_config.yml の exclude に " + name + " が無い",
    );
  }
  /* exclude は Jekyll の既定を置き換えてしまうので、既定分も残っている必要がある。
	   ここが消えると、うっかり npm install したときに node_modules が公開される */
  assert.ok(
    excluded.includes("node_modules"),
    "_config.yml の exclude から node_modules が消えている",
  );
});
...
test("公開に必要なファイルが exclude に巻き込まれていない", () => {
  ...
  for (const f of needed) {
    for (const ex of excluded) {
      assert.ok(
        f !== ex && !f.startsWith(ex + "/"),
        f +
          " が _config.yml の exclude（" +
          ex +
          "）に当たり、公開されなくなる",
      );
    }
