...
test("特例の限度面積と減額割合", () => {
  ...
  assert.strictEqual(SMALL_LOT_RATE, 0.8);
});
...
test("特例の減額：330㎡を超える分は対象外", () => {
  ...
  near(
    smallLotReduction(5000, 330.001),
    smallLotReduction(5000, 330),
    0.05,
    "境目",
  );
});
...
test("特例も非課税枠も使わなければ、これまでと同じ結果になる", () => {
  ...
  const zeroOpts = simulate(20000, 3000, true, 2, 50, 0, 5, {
    land: { value: 0, area: 200, first: true, second: true },
    insurance: 0,
  });
  assert.deepStrictEqual(
    zeroOpts,
    withoutArg,
    "既存の共有URLの結果が変わってはいけない",
  );
...
test("特例を使うと税額は必ず下がる（同じ条件の比較）", () => {
  ...
  const off = simulate(...base, {
    land: { value: 8000, area: 200, first: false, second: false },
  });
  const on1 = simulate(...base, {
    land: { value: 8000, area: 200, first: true, second: false },
  });
  const both = simulate(...base, {
    land: { value: 8000, area: 200, first: true, second: true },
  });
  assert.ok(on1.first < off.first, "一次で特例を使っても税額が下がっていない");
  assert.ok(
    both.second < on1.second,
    "二次で特例を使っても税額が下がっていない",
  );
  assert.ok(
    both.grand < on1.grand && on1.grand < off.grand,
    "合計税額の大小関係",
  );
});
...
test("特例の減額は遺産額を超えない", () => {
  ...
  const r = simulate(3000, 0, true, 1, 100, 0, 10, {
    land: { value: 9000, area: 100, first: true, second: true },
  });
...
test("配偶者がいない場合でも一次の特例は効く", () => {
  const off = simulate(20000, 0, false, 1, 0, 0, 10, {
    land: { value: 5000, area: 200, first: false, second: false },
  });
  const on = simulate(20000, 0, false, 1, 0, 0, 10, {
...
test("特例を使っても、取得割合を動かした税額合計は有限で正", () => {
  ...
  for (let p = 0; p <= 100; p++) {
    ...
    assert.ok(
      Number.isFinite(r.grand) && r.grand >= 0,
      p + "%で税額合計が不正",
    );
