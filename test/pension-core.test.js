...
const { pRate, calcW, leaderTimeline, MINA, MAXA, CALCA, MAPM } =
	require('../pension/js/pension-core.js');

function near(actual, expected, tol, msg) {
	assert.ok(Math.abs(actual - expected) <= (tol === undefined ? 1e-9 : tol),
...
test('calcW：損益分岐する年齢は解析解と一致する', () => {
	...
	const w60 = calcW(60, 0, 60, 0), w65 = calcW(65, 0, 60, 0);
...
test('leaderTimeline：期間が隙間なく並ぶ', () => {
	...
	near(periods[periods.length - 1].end, CALCA, 1e-9, '最後の期間は計算の上限まで');
	for (let i = 1; i < periods.length; i++) {
		near(periods[i].start, periods[i - 1].end, 1e-9, i + '番目の期間が前の期間と繋がっていない');
...
test('leaderTimeline：利回り0%なら交差する年齢は解析解と一致する', () => {
	...
	assert.deepStrictEqual(periods.map(p => ages[p.idx]), [60, 65, 70, 75]);
