...
test("住民税の所得割は10%", () => {
  assert.strictEqual(INHABITANT_TAX_RATE, 0.1);
...
test("退職所得の税額：控除額以下なら課税されない", () => {
  assert.deepStrictEqual(
    calcTax(8000000, 8000000),
    { tax: 0, inhabitTax: 0 },
    "控除額ちょうど",
  );
  assert.deepStrictEqual(
    calcTax(5000000, 8000000),
    { tax: 0, inhabitTax: 0 },
    "控除額未満",
  );
  assert.deepStrictEqual(
...
test("退職所得の税額：所得税の速算表と同じ表を使っている", () => {
  ...
  taxNear(
    calcTax(koujo + 80000000, koujo).tax,
    incomeTax(40000000),
    "40%の上限",
  );
});
...
test("短期退職手当等：300万円を超える部分は2分の1にならない", () => {
  ...
  assert.strictEqual(
    retireIncome(8000000, 2000000, 6),
    3000000,
    "6年なら普通に半分",
  );
});
...
test("公的年金等：65歳の前後で控除が変わる", () => {
  ...
  assert.ok(
    pensionMiscIncome(2000000, 65) < pensionMiscIncome(2000000, 64),
    "65歳以上のほうが控除が大きいはず",
  );
  assert.strictEqual(pensionMiscIncome(2000000, 64), 2000000 * 0.75 - 275000);
  assert.strictEqual(pensionMiscIncome(2000000, 65), 900000);
...
test("公的年金等：410万円から上は年齢によらず同じ", () => {
  for (const revenue of [
    4100000, 5000000, 7700000, 9000000, 10000000, 15000000,
  ]) {
    assert.strictEqual(
      pensionMiscIncome(revenue, 60),
      pensionMiscIncome(revenue, 70),
