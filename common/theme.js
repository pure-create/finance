...
(function (global) {
  ...
  var darkQuery = global.matchMedia
  ...
  function itemFor(m) {
    for (var i = 0; i < ITEMS.length; i++)
      if (ITEMS[i].mode === m) return ITEMS[i];
    return ITEMS[0];
  }
  ...
  function updateWidgets() {
    ...
    for (var i = 0; i < widgets.length; i++) {
      ...
      w.btn.setAttribute(
        "aria-label",
        MENU_TITLE + ": " + cur.name + "（" + cur.desc + "）",
      );
      /* 吹き出しの文言。ブラウザ標準の title は使わない（出るまでが遅く、
			   見た目もサイト内の他の吹き出しと揃わないため）。両方あると
			   二重に出るので、title は付けないこと */
      w.tip.textContent =
        MENU_TITLE + "を変更できます（現在の設定：" + cur.name + "）";
      for (var j = 0; j < w.items.length; j++) {
        w.items[j].setAttribute(
          "aria-checked",
          w.items[j].dataset.mode === mode ? "true" : "false",
        );
      }
    }
  }
