...
function statutoryPct() {
  return state.hasSpouse && state.nChildren > 0 ? 50 : 100;
}
function getAssetMe() {
  return Math.max(0, +$("assetMe").value || 0);
}
function getAssetSp() {
  const el = $("assetSp");
...
function getDelta() {
  return +$("spDelta").value || 0;
}
function getYears() {
  return Math.min(10, Math.max(0, Math.floor(+$("yearsGap").value || 0)));
}
function getLandValue() {
  return Math.max(0, +$("landValue").value || 0);
}
function getLandArea() {
  return Math.max(0, +$("landArea").value || 0);
}
function getInsurance() {
  return Math.max(0, +$("insurance").value || 0);
}
function heirCount() {
  return (state.hasSpouse ? 1 : 0) + state.nChildren;
}
...
function okuText(v) {
  ...
  const oku = Math.floor(a / 10000),
    man = Math.round(a % 10000);
  return (
    "= " + (v < 0 ? "−" : "") + oku + "億" + (man ? fmt(man) + "万" : "") + "円"
  );
}
...
function collectState() {
  return {
    me: getAssetMe(),
    sp: getAssetSp(),
    s: state.hasSpouse ? 1 : 0,
    c: state.nChildren,
    pct: state.spSharePct,
    mv: state.userMoved ? 1 : 0,
    d: getDelta(),
    y: getYears(),
    lv: getLandValue(),
    la: getLandArea(),
    ins: getInsurance(),
    l1: $("landFirst").checked ? 1 : 0,
    l2: $("landSecond").checked ? 1 : 0,
  };
}
function saveState() {
  try {
...
function loadState() {
  ...
  if (q.has("me")) {
    ...
    try {
      src = JSON.parse(localStorage.getItem(LS_KEY));
    } catch (e) {}
  }
  if (!src || typeof src !== "object") return null;
  if (src.s != null) state.hasSpouse = src.s !== 0;
  if (Number.isFinite(src.c))
    state.nChildren = Math.min(6, Math.max(1, Math.round(src.c)));
  state.userMoved = src.mv === 1;
  if (state.userMoved && Number.isFinite(src.pct))
    state.spSharePct = Math.min(100, Math.max(0, src.pct));
  return src;
}
/* ---------- 家族の図の描画 ---------- */
...
function drawTree() {
  ...
  const kids = [...fam.querySelectorAll(".person.child")]
    .map((el) => {
...
const markerPlugin = {
  ...
  afterDatasetsDraw(c) {
    ...
    if (state.bestLo != null && !(state.bestLo === 0 && state.bestHi === 100)) {
      ...
      ctx.strokeStyle = Theme.color("--inh-keep");
      ctx.lineWidth = 1.5;
      ctx.setLineDash([3, 3]);
      ctx.beginPath();
      ctx.moveTo(xLo, top);
      ...
      const cx = Math.min(
        Math.max((xLo + xHi) / 2, c.chartArea.left + 40),
        c.chartArea.right - 40,
      );
...
function renderChart(sweep) {
  ...
  if (!sweep || typeof Chart === "undefined") {
  ...
  for (let p = 0; p <= 100; p++) {
    labels.push(p);
    d1.push(sweep[p].first);
    d2.push(sweep[p].second);
    dg.push(sweep[p].grand);
  }
...
function updateLandCard(land, assetMe) {
  ...
  if (on && land.value > assetMe) {
    ...
    note.textContent =
      "自宅の土地の評価額が「自分の資産額」を超えています。自宅の土地は資産額に含めて入力してください";
    ...
    note.textContent =
      "今は二次相続に特例を適用しない設定です。一次相続と違って自動では適用されませんが、同居していた子のほか、持ち家のない子が相続する場合も対象になり得ます";
  } else {
    note.className = "assume-note";
    note.textContent = "";
  }
...
function updateInsuranceCard(payout, assetMe) {
  ...
  limit.innerHTML =
    ...
    (payout > 0
      ? `<span class="ins-used">このうち <b>${fmt(used)}万円</b> が課税価格から外れます</span>`
      : "");
  ...
  if (payout > 0 && payout > assetMe) {
    note.className = "assume-note warn";
    note.textContent =
      "死亡保険金が「自分の資産額」を超えています。保険金は資産額に含めて入力してください";
  } else if (payout > cap) {
    note.className = "assume-note";
...
function update() {
  ...
  $("grandLabel").textContent = state.hasSpouse
    ? "一次＋二次の相続税合計"
    : "相続税の合計";
  ...
  $("yearsGap").setAttribute(
    "aria-valuetext",
    years >= 10 ? "10年以上" : `${years}年`,
  );
  ...
  if (state.hasSpouse) {
    ...
    if (neverApplies) {
      ...
      const thPct =
        Math.floor((Math.max(16000, r.taxable1 / 2) / r.taxable1) * 100) + 1;
      $("yearsNote").textContent =
        `現在の取得割合では一次相続で配偶者に税額が発生しません（${thPct}%以上で相次相続控除が影響）`;
    } else {
      $("yearsNote").textContent =
        "10年未満の場合、相次相続控除（一次で配偶者が納めた税額の一部を二次で控除）を適用します";
    }
  }
  ...
  if (showSplit) {
    ...
    $("legendCh").innerHTML =
      `子 ${fmt(chAmt)}万円` +
      (state.nChildren > 1
        ? `（1人あたり <b class="pc-num">${fmt(chAmt / state.nChildren)}万円</b>）`
        : "");
    ...
    const EPS = 1e-6;
    let lo = 0;
    while (grands[lo] > bestV + EPS) lo++;
    let hi = lo;
    while (hi < 100 && grands[hi + 1] <= bestV + EPS) hi++;
    let rangeText, jumpP;
    if (lo === 0 && hi === 100) {
      rangeText = "どの割合でも同額";
      jumpP = statutoryPct();
    ...
    state.bestLo = lo;
    state.bestHi = hi;
    state.bestText = rangeText;

    let wLo = 0;
    while (grands[wLo] < worstV - EPS) wLo++;
    let wHi = wLo;
    while (wHi < 100 && grands[wHi + 1] >= worstV - EPS) wHi++;
    let worstRangeText;
  ...
  if (r.taxable1 <= ded1)
    h1 += `<div class="zero-note">基礎控除以下のため相続税はかかりません</div>`;
  ...
  if (state.hasSpouse) {
    ...
    if (r.taxable2 <= ded2)
      h2 += `<div class="zero-note">基礎控除以下のため相続税はかかりません</div>`;
...
if (saved) {
  ...
  if (Number.isFinite(saved.y))
    $("yearsGap").value = Math.min(10, Math.max(0, saved.y));
