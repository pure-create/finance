...
// 共有URLのQRコードを描画する（スマートフォンへの共有用。PC表示時のみ）
  ...
  if (state.hasSpouse) {
    ...
    slot.innerHTML = `
      <div class="person sp">
        ...
        <svg class="avatar" viewBox="0 0 48 48" fill="#db2777" aria-hidden="true" focusable="false"><circle cx="24" cy="15" r="9"/><path d="M24 27c-10 0-16 6-16 14h32c0-8-6-14-16-14z"/></svg>
  ...
  for (let i = 0; i < state.nChildren; i++) {
    html += `
      <div class="person child">
        ...
        <svg class="avatar" viewBox="0 0 48 48" fill="#059669" aria-hidden="true" focusable="false"><circle cx="24" cy="15" r="9"/><path d="M24 27c-10 0-16 6-16 14h32c0-8-6-14-16-14z"/></svg>
...
function drawTree() {
  ...
  svg.innerHTML = `<path d="${d}" stroke="#94a3b8" stroke-width="2" fill="none"/>`;
}
...
const markerPlugin = {
  ...
  afterDatasetsDraw(c) {
    ...
    if (state.bestLo != null && !(state.bestLo === 0 && state.bestHi === 100)) {
      ...
      ctx.strokeStyle = '#059669'; ctx.lineWidth = 1.5; ctx.setLineDash([3, 3]);
...
function renderChart(sweep) {
  ...
  const data = {
    labels,
    datasets: [
      { label: '一次相続の税額', data: d1, borderColor: '#1d4ed8', backgroundColor: '#1d4ed8', borderWidth: 2, pointRadius: 0, tension: .15 },
      { label: '二次相続の税額', data: d2, borderColor: '#db2777', backgroundColor: '#db2777', borderWidth: 2, pointRadius: 0, tension: .15 },
      { label: '合計', data: dg, borderColor: '#fff', backgroundColor: '#fff', borderWidth: 7, pointRadius: 0, tension: .15, order: 1, _halo: true },
      { label: '合計', data: dg, borderColor: '#0f172a', backgroundColor: '#0f172a', borderWidth: 4, pointRadius: 0, tension: .15, order: 0 },
    ]
  };
  ...
  chart = new Chart($('chart'), {
    ...
    options: {
      ...
      scales: {
        x: {
          title: { display: true, text: '配偶者の取得割合（％）', font: { size: 11 } },
          ticks: {
            font: { size: 11 }, autoSkip: false, maxRotation: 0,
            callback: v => v % 10 === 0 ? v + '%' : null,
          },
          grid: { display: false },
        },
        y: {
          title: { display: true, text: '税額（万円）', font: { size: 11 } },
      ...
      plugins: {
        legend: {
          labels: {
            font: { size: 12 }, boxWidth: 14, boxHeight: 3,
...
$('shareBtn').onclick = () => {
  ...
  const flash = msg => {
    const b = $('shareLabel');
    b.textContent = msg;
    setTimeout(() => { b.textContent = '試算結果を共有'; }, 1600);
  };
  copyText(url).then(() => flash('試算結果のURLをコピーしました ✓'), () => flash('コピーできませんでした'));
};
