...
const FIELDS = [
  ...
  ["joinAge", 40, "ja"],
  ["payAge", 65, "pa"],
  ["retireAmount", 2000, "ra"],
  ["hireAge", 22, "ha"],
  ["retireAge", 60, "rt"],
  ["annuityYears", 10, "ay"],
  ["publicPension", 180, "pp"],
  ["publicPensionAge", 65, "ps"],
  ["mixRatio", 50, "mx"],
];
...
const markerPlugin = {
  ...
  afterDatasetsDraw(c) {
    ...
    if (state.bestAge != null) {
      ...
      const cx = Math.min(
        Math.max(x, c.chartArea.left + pad),
        c.chartArea.right - pad,
      );
...
const mixMarkerPlugin = {
  ...
  afterDatasetsDraw(c) {
    ...
    if (state.mixLo != null && !(state.mixLo === 0 && state.mixHi === 100)) {
      ...
      ctx.beginPath();
      ctx.moveTo(xLo, top);
      ctx.lineTo(xLo, bottom);
      ctx.stroke();
      if (xHi - xLo > 2) {
        ctx.beginPath();
        ctx.moveTo(xHi, top);
        ctx.lineTo(xHi, bottom);
        ctx.stroke();
      }
      ctx.setLineDash([]);
      ctx.fillStyle = Theme.color("--idc-gain");
      ...
      const pad = ctx.measureText(label).width / 2 + 4;
      const cx = Math.min(
        Math.max((xLo + xHi) / 2, c.chartArea.left + pad),
        c.chartArea.right - pad,
      );
      ctx.fillText(label, cx, top + 4);
      ctx.restore();
...
function renderMixChart(b) {
  if (typeof Chart === "undefined") {
    $("mixChart").parentNode.style.display = "none";
    return;
  }
  const labels = [];
        ...
        tooltip: {
          ...
          callbacks: {
            title: (items) =>
              ...
              "%",
            label: (item) =>
              item.dataset.label + "：" + fmt(item.parsed.y) + "万円",
          },
          backgroundColor: Theme.color("--tooltip-bg"),
          titleColor: Theme.color("--tooltip-text"),
          bodyColor: Theme.color("--tooltip-text"),
...
function describeRule(lump) {
  ...
  if (lump.sameYear) {
    ...
    box.innerHTML =
      "同じ年に両方を受け取るため、<b>合算して1回分の退職所得控除</b>になります" +
      "（重なる" +
      lump.overlap +
      "年を除いた通算" +
      (lump.idecoYears + lump.retireYears - lump.overlap) +
      "年で計算）。";
    return;
  }
  if (lump.adjusted === "ideco") {
    ...
    const escape =
      needAge <= PAYOUT_AGE_MAX
        ? "削られた控除を満額に戻すには、iDeCoの受け取りを<b>" +
          needAge +
          "歳まで遅らせる</b>必要があります。"
        : "削られた控除を満額に戻すには、iDeCoの受け取りを" +
          needAge +
          "歳まで遅らせる必要がありますが、受給は" +
          PAYOUT_AGE_MAX +
    ...
    box.innerHTML =
      ...
      Math.abs(lump.gap) +
      "年後にiDeCoを受け取ります。" +
      "退職金が先の場合は<b>前年以前19年内</b>が対象なので、重なる" +
      lump.overlap +
      "年分、<b>iDeCo側の控除が削られます</b>。" +
      escape;
    return;
  }
  if (lump.adjusted === "retire") {
    ...
    const escape =
      needAge >= PAYOUT_AGE_MIN
        ? "iDeCoの受け取りを<b>" +
          needAge +
          "歳まで早めれば</b>、10年以上空いて調整はなくなります。"
        : "調整を外すには退職金の10年以上前に受け取る必要がありますが、iDeCoの受給は" +
          PAYOUT_AGE_MIN +
          "歳からなので、この退職年齢では避けられません。";
    box.className = "note rule-note";
    box.innerHTML =
      "iDeCoを先に受け取り、" +
      Math.abs(lump.gap) +
      "年後に退職金を受け取ります。" +
      "iDeCoが先の場合は<b>前年以前9年内</b>が対象なので、重なる" +
      lump.overlap +
      "年分、<b>退職金側の控除が削られます</b>。" +
      escape;
    return;
  }
...
function update() {
  ...
  if (cfg.payAge < earliest) {
    ...
    payWarn.textContent =
      cfg.idecoJoinAge >= LATE_JOIN_AGE
        ? LATE_JOIN_AGE +
          "歳以降に加入した場合、受け取れるのは加入から" +
          LATE_JOIN_WAIT +
          "年後の" +
          earliest +
          "歳からです。" +
  ...
  const limitNow = contributionLimit(
    cfg.category,
    THIS_YEAR,
    cfg.otherPlanMonthly,
    ageIn(THIS_YEAR),
  );
  const limitNext = contributionLimit(
    cfg.category,
    LIMIT_REFORM_YEAR,
    cfg.otherPlanMonthly,
    ageIn(LIMIT_REFORM_YEAR),
  );
  let limitHtml =
    limitNow > 0
      ? "今年の上限 <b>" + fmt(limitNow) + "円/月</b>"
      : "今年は拠出できません";
  ...
  if (endsAt60 && limitNow === 0) {
    warns.push(
      "第1号・第3号被保険者は60歳未満までなので、今年は拠出できません。" +
        LIMIT_REFORM_YEAR +
        "年からは60歳以上70歳未満でも「第5号加入者」として月" +
        fmt(LATE_JOIN_CATEGORY_LIMIT) +
        "円まで拠出できます。",
    );
  } else if (
    endsAt60 &&
    cfg.startAge < NATIONAL_PENSION_END_AGE &&
    cfg.payAge > NATIONAL_PENSION_END_AGE
  ) {
    warns.push(
      "第1号・第3号被保険者は60歳未満までです。60歳以降は「第5号加入者」として月" +
        fmt(LATE_JOIN_CATEGORY_LIMIT) +
        "円までの枠で計算します。",
    );
  }
  ...
  if (limitNow > 0 && cfg.monthly > limitNow && cfg.monthly <= limitNext) {
    warns.push(
      "今の掛金は今年の上限（" +
        ...
        "円）を超えています。" +
        LIMIT_REFORM_YEAR +
        "年からは出せる額ですが、それまでは上限までで計算します。",
    );
  } else if (cfg.monthly > limitNext && limitNext > 0) {
    warns.push(
      "今の掛金は改正後の上限（" +
        fmt(limitNext) +
        "円）も超えています。上限までで計算します。",
    );
  }
  ...
  if (!(cfg.initialBalance > 0)) {
    ...
    paidNote.className = "note";
    paidNote.innerHTML =
      "未入力なので、残高すべてを元本（含み益なし）として計算します";
  } else if (initGain >= 0) {
    paidNote.className = "note";
    paidNote.innerHTML =
      "含み益 <b>" + man(initGain) + "万円</b> として計算します";
  } else {
  ...
  if (cfg.publicPension > 0 && pRate !== 1) {
    ...
    pensionNote.innerHTML =
      cfg.publicPensionStartAge +
      "歳からなら <b>" +
      man(cfg.publicPension * pRate) +
      "万円/年</b>（" +
      (pRate > 1 ? pct + "%増" : pct + "%減") +
      "）";
  } else {
  ...
  const acc = accumulate(
    {
      ...
      monthly: cfg.monthly,
      otherPlanMonthly: cfg.otherPlanMonthly,
      yieldRate: cfg.yieldRate,
      taxableIncome: cfg.taxableIncome,
      initialBalance: cfg.initialBalance,
      initialPaid: cfg.initialPaid,
    },
    Tax,
  );
  ...
  function diffNote(r, lead) {
    ...
    const base =
      "iDeCoを受け取らなければ、退職金にかかる税額は <b>" +
      yen(r.taxWithoutIdeco) +
      "</b> でした。";
  ...
  if (L.sameYear) {
    ...
    h =
      ...
      rline(
        ...
        "−" + yen(L.ideco.deduction),
        "dim",
      ) +
      rline("税額", "−" + yen(idecoTax), "dim") +
      rline(
        "iDeCoの手取り",
        yen(idecoAmount - idecoTax),
        hasRetire ? "sub" : "total",
      );
    if (hasRetire) {
      h += retireLines(
  ...
  if (L.shortTenure) {
    h +=
      ...
      (L.sameYear ? "合算した勤続年数が" : "加入期間が") +
      L.shortTenureYears +
      "年しかないため<b>短期退職手当等</b>にあたり、" +
      "退職所得控除を引いた残りのうち" +
      man(Tax.SHORT_TENURE_HALF_LIMIT) +
      "万円を超える部分は半分になりません。</div>";
  }
  if (L.tax === 0)
    h += '<div class="zero-note">控除の範囲に収まるため非課税です</div>';
  if (hasRetire) {
    h += diffNote(
      L,
      L.sameYear
  ...
  if (d.growth > 0) {
    h2 +=
      rline("受取中の運用で増える分", "＋" + yen(d.growth), "dim") +
  ...
  h2 +=
    rline(
      "増える雑所得（年）",
      miscChanges ? yen(miscFrom) + " → " + yen(miscTo) : yen(miscFrom),
      "dim",
    ) +
  ...
  if (miscChanges) {
    h2 +=
      '<div class="split-note">老齢年金が出はじめる' +
      d.pensionAge +
  ...
  $("segLump").textContent =
    mixPct >= 20 ? "一時金 " + man(mix.lumpAmount) + "万円" : "";
  $("segAnnuity").textContent =
    mixPct <= 80 ? "年金 " + man(mix.annuityAmount) + "万円" : "";
  ...
  if (!bandText) {
    $("mixBestHint").innerHTML =
      "この条件では、割合をどう変えても増える税金は <b>" +
      man(bm.best) +
      "万円</b> のままです。";
  } else {
    const inBest = mixPct >= bm.lo && mixPct <= bm.hi;
    $("mixBestHint").innerHTML =
      '<span class="tag">最小</span>一時金を <b>' +
      bandText +
      "</b> にすると、iDeCoによって増える税金は <b>" +
      man(bm.best) +
      "万円</b>" +
      (mixWins
        ? "（一時金だけ・年金だけの良いほうより <b>" +
          ...
          "万円</b> 少ない）"
        : "") +
      '<button id="mixJumpBest" type="button"' +
      (inBest ? " disabled" : "") +
      ">この割合にする</button>";
    if (!inBest) {
      $("mixJumpBest").onclick = () => {
        inputs.set("mixRatio", bm.lo);
        update();
      };
    }
  }
  ...
  if (mix.lump) {
    /* 同じ年に退職金と受け取る場合、控除は通算の勤続年数で1回分。
		   別の年でも、重なる期間があれば削られる */
    const adj = mix.lump.sameYear
      ? "（通算）"
      : mix.lump.adjusted === "ideco"
        ? "（調整後）"
        : "";
    h3 += rline(
      "退職所得控除" + adj,
      "−" + yen(mix.lump.ideco.deduction),
      "dim",
    );
  }
  ...
  if (!mix.lump) {
    ...
    h3 +=
      '<div class="zero-note">一時金で受け取らないので、退職所得控除は使いません' +
  ...
  if (mix.annuityAmount > 0) {
    h4 += rline(
      "1年あたりの受取額",
      man(md.perYear) + "万円 × " + md.years + "年",
      "dim",
    );
    if (md.growth > 0)
      h4 += rline("受取中の運用で増える分", "＋" + yen(md.growth), "dim");
    const mFrom = md.rows.length ? md.rows[0].misc : 0;
    const mTo = md.rows.length ? md.rows[md.rows.length - 1].misc : 0;
    h4 += rline(
      "増える雑所得（年）",
      man(mFrom) !== man(mTo) ? yen(mFrom) + " → " + yen(mTo) : yen(mFrom),
      "dim",
    );
  }
  h4 += mixTaxRow(md.tax);
  ...
  const sub =
    '<span class="gsub">もう一方より ' +
    man(gapTax) +
    "万円 少ない（iDeCoを受け取らない場合との差）</span>";
  if (mixWins) {
    $("grandLabel").innerHTML =
      (taxDown ? "税金が減るのは " : "増える税金が最も少ないのは ") +
      "<b>" +
      mixName(bm.lo, bm.hi) +
      "</b>" +
      '<span class="gsub">一時金だけ・年金だけで受け取るより ' +
      man(mixGain) +
      "万円 少ない（iDeCoを受け取らない場合との差）</span>";
  } else {
    $("grandLabel").innerHTML =
  ...
  $("mixPointer").innerHTML = mixWins
    ? "一時金と年金を組み合わせると、どちらか一方で受け取るより <b>" +
      man(mixGain) +
  ...
  for (let age = earliest; age <= PAYOUT_AGE_MAX; age++) {
    ...
    sweep.push({
      age: age,
      lump: c.lump.taxByIdeco,
      annuity: c.annuity.taxByIdeco,
      mix: bmAge.best,
      mixText: mixRangeText(bmAge.lo, bmAge.hi) || "割合を問わず",
      mixLo: bmAge.lo,
      mixHi: bmAge.hi,
    });
  }
  ...
  for (const s of sweep) {
    ...
    if (s.mix < best - 5000) {
      best = s.mix;
      bestAge = s.age;
      bestName = mixName(s.mixLo, s.mixHi);
    }
  }
