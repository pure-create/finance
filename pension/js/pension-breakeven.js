...
function scColor(sc) {
  return Theme.color(sc.token);
}
function scBg(sc) {
  return Theme.color(sc.token + "-bg");
}
...
function render() {
  ...
  document.getElementById("rAfter").textContent = (
    nr *
    (1 - tax) *
    100
  ).toFixed(2);
  document.getElementById("rTaxNote").textContent = taxOn
    ? `（課税率 ${(TAX_RATE * 100).toFixed(3)}%）`
    : "（非課税）";
  ...
  const datasets = SC.map((sc, i) => ({
    label: sc.label,
    data: chartAges.map((a) => ({
      x: a,
      y: +ws[i][(a - MINA) * 12].toFixed(3),
    })),
        ...
        plugins: {
          tooltip: {
            callbacks: {
              ...
              label: (item) =>
                `${item.dataset.label}：${item.parsed.y.toFixed(2)}年分`,
            },
            titleFont: { size: 12 },
            bodyFont: { size: 11 },
...
Theme.onChange(() => {
  if (myChart) {
    ...
    o.scales.x.ticks.color = cSub;
    o.scales.x.grid.color = cGrid;
    o.scales.x.border.color = cGrid;
    o.scales.y.ticks.color = cSub;
    o.scales.y.grid.color = cGrid;
    o.scales.y.border.color = cGrid;
